    ...
    if(remember)
    {
        for(int i = 0; i < ui->treeWidgetSummary->topLevelItemCount(); i++)
        {
            ...
            for(int j = 0; j < ui->treeWidgetSummary->topLevelItem(i)->childCount(); j++)
            {
                ...
                {
                    rememberOpenedItemsID.append(ui->treeWidgetSummary->topLevelItem(i)->child(j)->child(k)->text(9));
...
void OverAid::on_comboBox_savedFilters_currentTextChanged()
{
    ...
    if(ui->comboBox_savedFilters->currentText() == "") {
        ...
        QSqlQuery update("UPDATE Filtres SET type='"+inOut+"', moyen='"+moyen+"', categorie='"+cat+"', sous_categorie='"+cat2+"', date_debut='"+ui->dateEditFilter_start->text()+"', date_fin='"+ui->dateEditFilter_end->text()+"', "
        ...
        if(filtres.next()) {
            ...
            if(ui->comboBoxFiltre_moyen->findText(moyenH) != -1) ui->comboBoxFiltre_moyen->setCurrentText(moyenH);
...
void OverAid::actu_filtre()
{
    ...
    where = inOut+" AND"+moyen+" AND"+cat+" AND"+cat2+" AND"+desc+" AND"+devise+" AND id_compte='"+QString::number(id_compte)+"' AND"+date+" ";
...
void OverAid::actu_historique()
{
    ...
    ui->treeWidgetSummary->hideColumn(9);
    ...
    while(transaction.next())
    {
        ...
        trans->setText(9, transaction.value("id_trans").toString());
    ...
    if(!rememberOpenedYears.isEmpty() && !rememberOpenedMonth.isEmpty() && !rememberOpenedItems.isEmpty())
    {
        for(int i = 0; i < ui->treeWidgetSummary->topLevelItemCount(); i++)
        {
            ...
            for(int j = 0; j < ui->treeWidgetSummary->topLevelItem(i)->childCount(); j++)
            {
                ...
                for(int k = 0; k < ui->treeWidgetSummary->topLevelItem(i)->child(j)->childCount(); k++)
                {
                    int item = rememberOpenedItemsID.indexOf(ui->treeWidgetSummary->topLevelItem(i)->child(j)->child(k)->text(9));
...
void OverAid::tout_etendre()
{
    ...
    {
        ui->treeWidgetSummary->resizeColumnToContents(i);
        ui->treeWidgetSummary->setColumnWidth(i, ui->treeWidgetSummary->columnWidth(i)*1.2);
    }
}
...
{
    if(!ui->treeWidgetSummary->currentItem()->text(9).isEmpty())
    {
        QSqlQuery abo("SELECT * FROM Transactions WHERE id_trans='"+ui->treeWidgetSummary->currentItem()->text(9)+"'");
        while (abo.next())
        {
            QSqlQuery ajouter;
            ajouter.prepare("INSERT INTO Abonnements (id_compte, type, moyen, categorie, sous_categorie, description, montant, detail_montant, fichier, renouvellement, dernier, devise, montantDeviseCompte) "
                              ...
                              "'"+abo.value("renouvellement").toString().left(2)+"', '"+locale.toDate(ui->treeWidgetSummary->currentItem()->parent()->parent()->text(0)+ui->treeWidgetSummary->currentItem()->parent()->text(0),"yyyyMMM").toString("yyyyMM")+"'), "
                              "'"+abo.value("devise").toString()+"', '"+abo.value("montantDeviseCompte").toString()+"';");
...
void OverAid::delete_trans()
{
    if(!ui->treeWidgetSummary->currentItem()->text(9).isEmpty())
    {
        ...
        {
            QSqlQuery remove("DELETE FROM Transactions WHERE id_trans='"+ui->treeWidgetSummary->currentItem()->text(9)+"'");
            actu(true, false);
        }
    }
    else QMessageBox::warning(this, tr("Erreur"), tr("Il ne s'agit pas d'une transaction."), tr("Fermer"));
...
void OverAid::on_treeWidgetSummary_itemClicked(QTreeWidgetItem *item, int column)
{
    ...
    if(column == 6 && item->parent())
    {
        ...
        QSqlQuery transaction("SELECT montant,devise,detail_montant,montantDeviseCompte,Devises.symbole FROM Transactions JOIN Devises ON Devises.code=Transactions.devise WHERE id_trans='"+item->text(9)+"'");
        if(transaction.next())
        {
    ...
    {
        QSqlQuery transaction("SELECT fichier FROM Transactions WHERE id_trans='"+item->text(9)+"'");
        if(transaction.next())
        {
            QByteArray pdf = transaction.value("fichier").toByteArray();
