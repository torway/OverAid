...
{
    QString path = QFileDialog::getOpenFileName(this, tr("Choisir un fichier"), QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation), tr("Tout (*.pdf)\nPDF (*.pdf)"));
...
void TransForm::on_pushButtonAdd_clicked()
{
    ...
    else
    {
        ...
        {
            query.prepare("INSERT INTO Abonnements (id_compte, type, moyen, categorie, sous_categorie, description, montant, detail_montant, fichier, renouvellement, dernier, devise, montantDeviseCompte) "
                              "VALUES ('"+QString::number(id_compte)+"', '"+type.replace("'","''")+"', '"+moyen+"', '"+cat+"', "
                              "'"+cat2+"', '"+desc+"', '"+montant+"', '"+ope+"',:pdf, '"+ui->dateEditAdd->date().toString("dd")+"', '"+QDate::currentDate().toString("yyyyMM")+"', '"+ui->comboBox_devise->currentText().left(3)+"', '"+montantDeviseCompte+"');");
        ...
        if(commande == "Modifier abo")
        {
            query.prepare("UPDATE Abonnements SET type='"+type.replace("'","''")+"', moyen='"+moyen+"', categorie='"+cat+"', sous_categorie='"+cat2+"', description='"+desc+"', montant='"+montant+"', detail_montant='"+ope+"'"
                          ", fichier=:pdf, renouvellement='"+ui->dateEditAdd->date().toString("dd")+"', devise='"+ui->comboBox_devise->currentText().left(3)+"', montantDeviseCompte='"+montantDeviseCompte+"' WHERE id_sub='"+ui->label_id->text()+"'");
