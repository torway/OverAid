...
void OverAid::update()
{
    ...
    QSqlQuery checkSettings("SELECT name FROM sqlite_master WHERE type='table' AND name='Settings'");
    if(!checkSettings.next())
    {
        QSqlQuery settings("CREATE TABLE 'Settings' ('version' TEXT NOT NULL DEFAULT '2.0', 'language' TEXT NOT NULL DEFAULT 'Français', 'defaultAccount' TEXT NOT NULL DEFAULT '1', 'showGroupFilter' TEXT NOT NULL DEFAULT 'true', 'showDebugWindow' TEXT NOT NULL DEFAULT 'false', 'geometry' TEXT NOT NULL DEFAULT '0;0;0;0', 'hiddenColumns' TEXT NOT NULL DEFAULT '11');");
        if(!settings.executedQuery().isEmpty())
        {
            QSqlQuery settings("INSERT INTO Settings (version, defaultAccount) SELECT '2.0',id_compte FROM Comptes LIMIT 1;");

            if(QFile(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)+"/settings.csv").exists()) QFile::remove(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)+"/settings.csv");
    ...
    if(versionDB < versionApp)
    {
        ...
        if(versionDB < QVersionNumber::fromString("2.2"))
        {
            QSqlQuery addSettingsShownColumns("ALTER TABLE Settings ADD hiddenColumns TEXT NOT NULL DEFAULT '11'");
            QSqlQuery addProject_trans("ALTER TABLE Transactions ADD projet TEXT NOT NULL DEFAULT ''");
            QSqlQuery addProject_abo("ALTER TABLE Abonnements ADD projet TEXT NOT NULL DEFAULT ''");
            QSqlQuery addModeSaisie("ALTER TABLE Transactions ADD modeSaisie TEXT NOT NULL DEFAULT 'Manuel'");
            QSqlQuery addProject_filtre("ALTER TABLE Filtres ADD projet TEXT NOT NULL DEFAULT ''");
            QSqlQuery addModeSaisie_filtre("ALTER TABLE Filtres ADD modeSaisie TEXT NOT NULL DEFAULT ''");
        }
    }
